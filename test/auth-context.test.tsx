...
const configMock = vi.hoisted(() => ({ loadRuntimeConfig: vi.fn() }));

vi.mock("@lib/api", () => ({ api: apiMock }));
vi.mock("@lib/config", () => ({
  appConfig: { apiUrl: "http://runtime-api.test" },
  loadRuntimeConfig: configMock.loadRuntimeConfig,
}));
...
describe("AuthProvider", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    configMock.loadRuntimeConfig.mockResolvedValue(undefined);
    apiMock.get.mockResolvedValue({ account });
    apiMock.post.mockResolvedValue({});
  });

  it("loads runtime config, restores the account, and wires refreshed tokens", async () => {
    render(
      <AuthProvider>
        <Consumer />
      </AuthProvider>,
    ...
    expect(configMock.loadRuntimeConfig).toHaveBeenCalledOnce();
    expect(apiMock.setUrl).toHaveBeenCalledWith("http://runtime-api.test");
